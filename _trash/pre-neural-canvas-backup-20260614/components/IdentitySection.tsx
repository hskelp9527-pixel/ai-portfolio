...
export const IdentitySection: React.FC<IdentitySectionProps> = ({ theme }) => {
  ...
  const p3 = "熟练掌握 Claude Code、Codex 等使用方式，有熟练搭建 OpenClaw 经历，能够完成智能体和 Skill 的设计与落地使用，并通过 AI 编程方式快速验证想法，形成可实际使用的应用原型。";
          ...
          <div className="text-left space-y-6">
            ...
            <div className={`hidden lg:block pt-8 border-t mt-8 transition-colors duration-300 ${theme === 'dark' ? 'border-white/10' : 'border-black/10'}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className={`p-4 rounded-[1.5rem] border transition-all duration-300 ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/5'}`}>
                  <Activity size={16} className={`mb-2 transition-colors duration-300 ${theme === 'dark' ? 'text-green-400' : 'text-green-500'}`} />
                  <p className={`text-xs font-bold mb-1 transition-colors duration-300 ${theme === 'dark' ? 'text-white' : 'text-[#1D1D1F]'}`}>LangGraph 智能体</p>
                  <p className={`text-xs transition-colors duration-300 ${theme === 'dark' ? 'text-gray-400' : 'text-[#1D1D1F]/60'}`}>Claude Code / Codex AI 辅助开发</p>
                </div>
                <div className={`p-4 rounded-[1.5rem] border transition-all duration-300 ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/5'}`}>
                  <Zap size={16} className={`mb-2 transition-colors duration-300 ${theme === 'dark' ? 'text-orange-400' : 'text-orange-500'}`} />
                  <p className={`text-xs font-bold mb-1 transition-colors duration-300 ${theme === 'dark' ? 'text-white' : 'text-[#1D1D1F]'}`}>OpenClaw 落地</p>
                  <p className={`text-xs transition-colors duration-300 ${theme === 'dark' ? 'text-gray-400' : 'text-[#1D1D1F]/60'}`}>前沿 AI 技术生产力探索</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`glass rounded-[3rem] p-8 sm:p-12 border shadow-sm transition-all duration-700 hover:shadow-xl ${cardBg}`}>
          <div className="space-y-6">
            ...
            <p className={`text-sm sm:text-base lg:text-lg leading-relaxed font-medium transition-colors duration-300 ${theme === 'dark' ? 'text-gray-100' : 'text-[#1D1D1F]/80'}`}>
              在个人成长方面，我更关注信息输入与复盘能力的提升，持续通过阅读、讨论与记录的方式，将零散经验整理为可复用的知识，为后续在 AI 与产品方向上的实践提供支撑。使用 Obsidian 积极复盘，定期总结现有情况和可优化项，积极探索 AI 前沿技术落地生产力。
            </p>
          </div>
        </div>
      </motion.div>
